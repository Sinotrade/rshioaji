...
Use this reference only when the user explicitly asks how to install the Shioaji plugin/skill in Claude, Codex, or another agent environment. Do not use it for normal Shioaji onboarding, account opening, API keys, signing, Python install, or project setup.
只有在使用者明確詢問如何在 Claude、Codex 或其他 agent 環境安裝 Shioaji plugin/skill 時才使用本文件。一般 Shioaji 入門、開戶、API 金鑰、簽署、Python 安裝或專案設定不要讀這份。
